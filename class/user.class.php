...
class user {
    ...
    public function getallsignupuser() {
        $ss__testimonials = "select * from `users` order by id desc";
        // echo("select * from b2b_users where id='$id'");exit();
...
public function b2buplevel($id) {
    ...
    public function getAll_b2b_users() {
        $ss__testimonials = "SELECT * FROM `b2b_users` where user_type <> 0 and uplevel_id='".$_SESSION['b2b_kpr_rech_id']."' ORDER BY id DESC";
        // echo("select * from b2b_users where id='$id'");exit();
        return $this->getExecute($ss__testimonials);
    }
    public function getAll_b2b_users_off() {
    ...
    public function getAll_b2b_users_admin() {
        $ss__testimonials = "SELECT * FROM `b2b_users` where (user_type=1  or user_type=4) and admin_id=1  ORDER BY id DESC";
        // echo("select * from b2b_users where id='$id'");exit();
        return $this->getExecute($ss__testimonials);
    }
    public function getAll_b2b_users_admin_off() {
        $con = mysqli_connect("localhost","paybay_main",'ZSE$6tfc',"paybay_main");
        global $prs_paging;
        $prs_paging = new get_paging();
        $parpage = (is_numeric(global_page_admin)) ? global_page_admin : '20';
